...
const ExpensesPage: React.FC = () => {
  ...
  // Get expense categories and ensure they're unique
  const allExpenseCategories = getCategoriesByType("expense");
  ...
  useEffect(() => {
    console.log("ExpensesPage - Categories:", expenseCategories.map(c => ({id: c.id, name: c.name})));
    console.log("ExpensesPage - Selected category:", selectedCategory);
  }, [expenseCategories, selectedCategory]);
  
  // Reset category selection if the selected category no longer exists
  useEffect(() => {
    if (selectedCategory !== 'all' && !expenseCategories.some(c => c.id === selectedCategory)) {
  ...
  const handleCategoryChange = (categoryId: string | 'all') => {
    console.log(`ExpensesPage - Setting category to: ${categoryId}`);
